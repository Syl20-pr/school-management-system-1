		 ...
		 <!-- Basic Forms -->
		  <div class="box">
			<div class="box-header with-border">
			  <h4 class="box-title">Modifier les Abandons</h4>
...
<div class="col-md-6">
 ...
 <option value="{{ $employee->id }}" {{ ($editData->employee_id == $employee->id)? 'selected':'' }}  >{{ $employee->name }}</option>
   ...
   <div class="form-group">
	<h5>Motif <span class="text-danger">*</span></h5>
	<div class="controls">
 ...
 <option value="{{ $leave->id }}" {{ ($editData->leave_purpose_id == $leave->id)?'selected':'' }}  >{{ $leave->name }}</option>
			 @endforeach
			 <option value="0">Nouveau Motif</option>
		</select>
<input type="text" name="name" id="add_another" class="form-control" placeholder="Écrire le Motif" style="display: none;">		
	 </div>
          </div>
   	 ...
   	 <div class="form-group">
		<h5>Date de Reprise <span class="text-danger">*</span></h5>
		<div class="controls">
