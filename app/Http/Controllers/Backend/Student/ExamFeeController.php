...
public function ExamFeeClassData(Request $request){
         ...
         $html['thsource'] .= '<th>Frais d\'Examen</th>';
         ...
         foreach ($allStudent as $key => $v) {
            ...
            $html[$key]['tdsource'] .='<a class="btn btn-sm btn-'.$color.'" title="PaySlip" target="_blanks" href="'.route("student.exam.fee.payslip").'?class_id='.$v->class_id.'&student_id='.$v->student_id.'&exam_type_id='.$request->exam_type_id.' ">Justificatif</a>';
            $html[$key]['tdsource'] .= '</td>';
...
public function ExamFeePayslip(Request $request){
 ...
 // dd($data['exam_type']);
    ...
    $pdf = PDF::loadView('backend.student.exam_fee.exam_fee_pdf', $data)->setPaper('a4')
        ->setOptions([
            'tempDir' => public_path(),
            'chroot' => public_path(),
        ]);
    //$pdf->SetProtection(['copy', 'print'], '', 'pass');
    return $pdf->download('Frai_Examen.pdf');
