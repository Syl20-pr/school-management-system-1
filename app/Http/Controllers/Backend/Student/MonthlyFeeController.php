...
public function MonthlyFeeClassData(Request $request){
         ...
         $html['thsource'] .= '<th>Frais Mensuel</th>';
         ...
         foreach ($allStudent as $key => $v) {
            ...
            $html[$key]['tdsource'] .='<a class="btn btn-sm btn-'.$color.'" title="PaySlip" target="_blanks" href="'.route("student.monthly.fee.payslip").'?class_id='.$v->class_id.'&student_id='.$v->student_id.'&month='.$request->month.' ">Justificatif</a>';
            $html[$key]['tdsource'] .= '</td>';
    ...
    $pdf = PDF::loadView('backend.student.monthly_fee.monthly_fee_pdf', $data)->setPaper('a4')
        ->setOptions([
            'tempDir' => public_path(),
            'chroot' => public_path(),
        ]);
    //$pdf->SetProtection(['copy', 'print'], '', 'pass');
    return $pdf->download('Frai_Mois.pdf');
    }
