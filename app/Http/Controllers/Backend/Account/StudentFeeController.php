...
class StudentFeeController extends Controller
{
  ...
  $data = AssignStudent::with(['discount'])->where('year_id',$year_id)->where('class_id',$class_id)->get();
         ...
         $html['thsource'] .= '<th>Nom du Père ou Tuteur</th>';
         $html['thsource'] .= '<th>Frais Initial </th>';
         $html['thsource'] .= '<th>Réduction</th>';
         $html['thsource'] .= '<th>FRAIS (De l\'Élève)</th>';
         $html['thsource'] .= '<th>Sélectionner</th>';
...
if($accountstudentfees !=null) {
    ...
    public function StudentFeeStore(Request $request){
        ...
        $notification = array(
            'message' => 'Données Mises à Jour Avec Succès. Bien Joué !',
            'alert-type' => 'success'
        );
        ...
        return redirect()->route('student.fee.view')->with($notification);
            ...
            $notification = array(
            'message' => 'Désolé, les Données n\'Ont Pas Été Enregistrées.',
            'alert-type' => 'error'
        );
