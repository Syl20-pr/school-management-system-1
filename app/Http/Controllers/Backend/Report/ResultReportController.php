...
class ResultReportController extends Controller
{
    ...
    $pdf = PDF::loadView('backend.report.student_result.student_result_pdf', $data)->setPaper('a4')
        ->setOptions([
            'tempDir' => public_path(),
            'chroot' => public_path(),
        ]);
    //$pdf->SetProtection(['copy', 'print'], '', 'pass');
    return $pdf->download('Rapport_Resultat_Eleves.pdf');
    }else{
        $notification = array(
            'message' => 'Désolé, Ces Critères Ne Correspondent Pas.',
            'alert-type' => 'error'
        );
    ...
    $pdf = PDF::loadView('backend.report.idcard.idcard_pdf', $data)->setPaper('a4')
        ->setOptions([
            'tempDir' => public_path(),
            'chroot' => public_path(),
        ]);
    //$pdf->SetProtection(['copy', 'print'], '', 'pass');
    return $pdf->download('ID_Carte_Eleve.pdf');
