...
class AttenReportController extends Controller
{
    ...
    if ($singleAttendance == true) {
        ...
        $data['leaves'] = EmployeeAttendance::with(['user'])->where($where)->where('attend_status','En Permission')->get()->count();

        $data['month'] = date('m-Y', strtotime($request->date));

    $pdf = PDF::loadView('backend.report.attend_report.attend_report_pdf', $data)->setPaper('a4')
        ->setOptions([
            'tempDir' => public_path(),
            'chroot' => public_path(),
        ]);
    //$pdf->SetProtection(['copy', 'print'], '', 'pass');
    return $pdf->download('Rapport_Permission.pdf');
